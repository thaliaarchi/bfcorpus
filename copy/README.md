# Brainfuck programs by copy

Brainfuck programs written by [copy](https://copy.sh/). They also made
[El Brainfuck](https://copy.sh/brainfuck/), a Brainfuck editor and optimizing
interpreter, a Brainfuck [text generator](https://copy.sh/brainfuck/text.html),
and a [program compressor](https://copy.sh/brainfuck/compress.html).

- [bfminifier.bf](https://copy.sh/stuff/brainfuck/bfminifier.bf)
- [div13.bf](https://copy.sh/stuff/brainfuck/div13.bf)
- [hammingweight.bf](https://copy.sh/stuff/brainfuck/hammingweight.bf)
- [euler06-brainfuck.bf](https://github.com/copy/project-euler/blob/master/euler06-brainfuck.bf)

TODO: Survey copy's [Code Golf solutions](https://codegolf.stackexchange.com/users/3428/copy).

License: None

## Third-party

- [calculator.bf](https://copy.sh/brainfuck/prog/calculator.bf) -> [../antosser/calculator.bf](../antosser/calculator.bf)
- [cellsize.bf](https://copy.sh/brainfuck/prog/cellsize.bf) -> [../esolang/brainfuck/cell_size.b](../esolang/brainfuck/cell_size.b)
- [dbf2c.b](https://copy.sh/brainfuck/prog/dbf2c.b) -> [../cristofd/dbf2c.b](../cristofd/dbf2c.b)
- [dquine.b](https://copy.sh/brainfuck/prog/dquine.b) -> [../cristofd/dquine.b](../cristofd/dquine.b)
- [mandelbrot.b](https://copy.sh/brainfuck/prog/mandelbrot.b) -> [../ebosman/mandelbrot/mandelbrot.b](../ebosman/mandelbrot/mandelbrot.b)
- [numwarp.b](https://copy.sh/brainfuck/prog/numwarp.b) -> [../cristofd/numwarp.old4.b](../cristofd/numwarp.old4.b)
- [random.b](https://copy.sh/brainfuck/prog/random.b) -> [../cristofd/random.b](../cristofd/random.b)
- [rot13.b](https://copy.sh/brainfuck/prog/rot13.b) -> [../cristofd/rot13.b](../cristofd/rot13.b)
- [squares.b](https://copy.sh/brainfuck/prog/squares.b) -> [../cristofd/squares.b](../cristofd/squares.b)
- [triangle.bf](https://copy.sh/brainfuck/prog/triangle.bf) -> [../nyyrikki/triangle.bf](../nyyrikki/triangle.bf)
- [wc.b](https://copy.sh/brainfuck/prog/wc.b) -> [../cristofd/wc.b](../cristofd/wc.b)

TODO:
- [beer.b](https://copy.sh/brainfuck/prog/beer.b)
- [bockbeer.b](https://copy.sh/brainfuck/prog/bockbeer.b)
- [char.bf](https://copy.sh/brainfuck/prog/char.bf)
- [hanoi.bf](https://copy.sh/brainfuck/prog/hanoi.bf)
- [hellom.bf](https://copy.sh/brainfuck/prog/hellom.bf)
- [oobrain.b](https://copy.sh/brainfuck/prog/oobrain.b)
- [PI16.BF](https://copy.sh/brainfuck/prog/PI16.BF)
- [quine505.b](https://copy.sh/brainfuck/prog/quine505.b)
- [yapi.b](https://copy.sh/brainfuck/prog/yapi.b)

Several programs are sourced from <https://esoteric.sange.fi/brainfuck/bf-source/prog/>.

TODO: Archive esoteric.sange.fi programs.
